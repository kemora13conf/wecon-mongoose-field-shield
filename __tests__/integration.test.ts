...
describe('Integration Tests', () => {
  ...
  describe('Multiple related models', () => {
    it('should handle separate policies for different models', async () => {
      ...
      expect(publicBook).toHaveProperty('title', 'Great Book');
      expect(publicBook).not.toHaveProperty('isbn');

      // Librarian can see ISBN
      const librarianBook = await Book.findOne().role('librarian');
      expect(librarianBook).toHaveProperty('title', 'Great Book');
      expect(librarianBook).toHaveProperty('isbn', '978-3-16-148410-0');
    });
  });
  ...
  describe('Complex conditions', () => {
    it('should handle organization-based access', async () => {
      ...
      hrResults.forEach((e: any) => {
        expect(e).toHaveProperty('salary');
      });
      ...
      // org1 employee matches (orgId === userId)
      const org1Employee = managerResults.find((e: any) => e.orgId === 'org-1');
      const org2Employee = managerResults.find((e: any) => e.orgId === 'org-2');

      expect(org1Employee).toHaveProperty('salary', 80000);
      expect(org2Employee).not.toHaveProperty('salary');
    });
  });
  ...
  describe('Real-world user model', () => {
    it('should correctly filter a typical user model', async () => {
      ...
      expect(publicView).toHaveProperty('username', 'johndoe');
      expect(publicView).toHaveProperty('avatar');
      expect(publicView).not.toHaveProperty('email');
      expect(publicView).not.toHaveProperty('phone');
      expect(publicView).not.toHaveProperty('salary');
      ...
      const otherUserView = await User.findById(user._id)
        .role('user')
        .userId('different-id');
      expect(otherUserView).toHaveProperty('username');
      expect(otherUserView).not.toHaveProperty('email'); // Condition failed
      expect(otherUserView).toHaveProperty('phone', '***-***-4567'); // Masked

      // Owner
      const ownerView = await User.findById(user._id)
        .role('user')
        .userId(user._id!.toString());
      expect(ownerView).toHaveProperty('email', 'john@example.com');
      expect(ownerView).toHaveProperty('phone', '***-***-4567'); // Still masked

      // Admin
      const adminView = await User.findById(user._id).role('admin');
      expect(adminView).toHaveProperty('username');
      expect(adminView).toHaveProperty('email');
      expect(adminView).toHaveProperty('phone', '555-123-4567'); // Full
      expect(adminView).toHaveProperty('salary', 100000);
      expect(adminView).toHaveProperty('createdAt');
      expect(adminView).not.toHaveProperty('password'); // Still hidden
      expect(adminView).not.toHaveProperty('refreshToken'); // Still hidden
    });
  });
});
